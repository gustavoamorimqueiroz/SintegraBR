            ...
            foreach (var property in listaComPropriedadesOrdenadas)
            {
                    ...
                    )
                {
                    ...
                    if (isRequired && isDecimal &&
                        ...
                        sb.Append(vZero.ToString("N" + sintegraCampoAttr.QtdCasas).Replace(",", "").PadLeft(sintegraCampoAttr.Tamanho, '0')); //.PadRight(sintegraCampoAttr.Tamanho, '0')); alterado por gustavo Right -> left 
                    else
                    {
                        if (isDecimal && hasValue)
                        {
                            ...
                            sb.Append(vDecimal.ToStringSafe().Replace(".", "").Replace(",", "").PadLeft(sintegraCampoAttr.Tamanho, '0'));//.PadRight(sintegraCampoAttr.Tamanho, '0'));alterado por gustavo Right -> left 
                        }
                        else
                        {
                            if (isNullableDecimal && hasValue)
                            {
                                var vDecimal =
                                    Convert.ToDecimal(propertyValue).ToString("N" + sintegraCampoAttr.QtdCasas);
                                sb.Append(vDecimal.ToStringSafe().Replace(".", "").Replace(",", "").PadLeft(sintegraCampoAttr.Tamanho, '0'));//.PadRight(sintegraCampoAttr.Tamanho, '0'));alterado por gustavo Right -> left 
                            ...
                            else
                            {
                                ...
                                {
                                    if (isNullableDateTime && hasValue)
                                    {
                                        sb.Append(Convert.ToDateTime(propertyValue).Date.ToString("yyyyMMdd"));
                                    }
                                    else
                                    {
                                        if (isCode && hasValue)
                                        {
                                            sb.Append(propertyValue.ToString().PadRight(sintegraCampoAttr.Tamanho, ' '));//.ReplaceDiacritics()); //adicionado ReplaceDiatric para retirada de caracteres especiais
                                        }
                                        else
                                        {
